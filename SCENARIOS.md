...
t=0 ─────────────────────── 1초 1건 페이스 ───────────────────────── t=15~30min
 │                                                                          │
 │  pool[0]=START ──→ pool[1]=START+1 ──→ ... ──→ pool[N-1]=START+VICTIM_COUNT-1 │
 │       │                       │                              │           │
 │       forge_token(START)      forge_token(START+1)           forge_token(...) (모두 새 jti)
...
| t=5min | **IP-사용자다양성(5분) override → 100** (1 IP × 다수 sub) | Slack 즉시 |
| t=5min | `/api/addresses` 비율 100% → F-Resp 70 | combined ≥ 90 |
| t=5min | ASN class = cloud_external (드문 회선) → ×1.2~1.5 | |
| t=5min~ | sub 단조성 추가 가중 (sub_id increment pattern) | |
| **MTTD 목표** | **5~15분** (충분한 신호량 위해 `VICTIM_COUNT` ≥ 300 권장) | |
...
```
t=0 ───────────── 1초 1건 페이스 (S4와 동일) ──────────── t=VICTIM_COUNT 초
 │                                                                  │
 │ sub=START → START+1 → ... → START+VICTIM_COUNT-1  (순차 순회)   │
 │     │              │              │                              │
 │     IP=rand(pool)  IP=rand(pool)  IP=rand(pool)                  │
 │                                                                  │
 └─ 총 VICTIM_COUNT건. duration 주면 페이스 자동, 아니면 1.0s sleep ─┘
```
...
| 총 호출 수 | **`VICTIM_COUNT`건 (S4·S5b와 동일)** | UBA factor 비교용 통일 |
...
- IP 풀(`VICTIM_COUNT × 0.68`개)이 골고루 호출 — 각 IP당 평균 약 1.5건
- **각 IP별로 보면 sub가 띄엄띄엄 흩어짐** (한 IP가 random 시점에 random sub 받음)
- **전 IP 합산하면 sub가 단조 증가** (`START` → `START+1` → ...)
...
|---|---|---|---|
| 총 호출 수 | `VICTIM_COUNT`건 | **`VICTIM_COUNT`건** | `VICTIM_COUNT`건 |
| 호출 페이스 | 1.0s | **1.0s (기본)** | 1.0s (기본) |
| IP 다양성 (전 구간) | 1 | `VICTIM_COUNT × 0.68` | `VICTIM_COUNT × 0.68` |
| 각 IP에서 본 sub 다양성 | 전부(순차) | **약 1.5건 평균** | 약 1.5건 평균 |
...
```
t=0 ─── 사전 추출된 VICTIM_COUNT명을 1회씩, IP만 매 요청 random ── t=VICTIM_COUNT 초
 │                                                                  │
 │ victims = [START,END) 중 VICTIM_COUNT명 랜덤 비복원 추출          │
 │ req#1   victim=victims[0]   ip=rand(ip_pool)   forge_token(...) │
 │ req#2   victim=victims[1]   ip=rand(ip_pool)   forge_token(...) │
 │   ...                                                            │
 │ req#VICTIM_COUNT (= 각 사용자 1회, S4·S5와 동일 총량)            │
 │                                                                  │
 └─ duration이 주어지면 페이스 자동, 아니면 1.0s sleep (S4 페이스) ──┘
```

victim을 범위에서 **랜덤 비복원 추출**(각 1회 방문), IP는 매 요청 random. 호출량은 S4/S5와
통일(`VICTIM_COUNT`건)해 UBA factor 차이를 깨끗하게 비교. **완전 분산** — IP·sub 선택 모두 random.
...
- IP 풀(`VICTIM_COUNT × 0.68`개) 골고루 — 각 IP당 평균 약 1.5건
- **sub가 랜덤 순서** — 순차 패턴 없음
- 계정당 정확히 1건 (비복원 추출 — 중복·누락 없음)
- 글로벌 시퀀스 분석해도 sub random 순서 — 단조성 없음

### S5와 S5b의 결정적 차이

| 차원 | S5 (sub 순차) | **S5b (sub random)** |
|---|---|---|
| 총 호출 수 | `VICTIM_COUNT`건 | `VICTIM_COUNT`건 (동일) |
...
를 한 줄로 입증 가능. 세 시나리오가 같은 호출량(`VICTIM_COUNT`건) · 같은 페이스(1.0s)로 통일되어 있으므로
...
| 가동 시간 | 30분 | 24시간 |
| 총 유출량 | `VICTIM_COUNT`건 | `VICTIM_COUNT`건 (동일) |
...
| **S4** | attacker 단독 | 단일 (AWS Seoul) | 순차 1회 (`VICTIM_COUNT`건) | `/api/addresses` 100% | 1.0s | forge_token (새 jti) | 단일 IP × 다수 sub |
| **S5** | attacker 단독 | 풀 random | **순차 1회 (`VICTIM_COUNT`건)** | `/api/addresses` 100% | 자동 / 1.0s | forge_token (새 jti) | IP만 분산 — **글로벌 sub 시퀀스에 잡힘** |
| **S5b** | attacker 단독 (botnet) | 풀 random | **랜덤 추출 1회씩 (`VICTIM_COUNT`건)** | `/api/addresses` 100% | 자동 / 1.0s | forge_token (새 jti) | **완전 분산 — 보완 factor만 잡음** |
...
- [ ] **S5**: 순차 1회 순회로 글로벌 sub 단조 패턴이 잡힐 만큼 신호가 충분한지 (`VICTIM_COUNT` 크기에 비례 — sample 100은 검증용, 발표 시연은 풀 전체 498 권장)
- [ ] **S5b**: 랜덤 비복원 추출(각 1회)이 맞는지, 쿠팡식 복원추출(한 계정 다회 조회) 분포가 별도 시나리오로 필요한지
